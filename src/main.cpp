...
struct ProgramContext
{
	...
	void initInput(void)
	{
		...
		{
			mem[0][i] = (float)random() / RAND_MAX;
			mem[1][i] = (float)random() / RAND_MAX;
...
int main(void)
{
	...
	ret = clGetPlatformInfo(pcx.platform_id, CL_PLATFORM_NAME, 4096, value, &rsize);
	printf("Platform: %s\n", value);
	ret = clGetPlatformInfo(pcx.platform_id, CL_PLATFORM_VERSION, 4096, value, &rsize);
